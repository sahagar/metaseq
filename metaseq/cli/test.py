...
def worker_main(cfg1: MetaseqConfig):
...
def index():
    ...
    fn = os.environ.get("DEMO_INDEXPAGE", os.path.join("metaseq", "service/index.html"))
    with open(fn) as f:
        return f.read()
...
def extra_args(parser):
    # add additional args
    parser.add_argument(
        "--num-gpus", type=int, required=True, help="number of GPUs per node"
    )
    parser.add_argument(
        "--num-nodes",
        type=int,
